...
# 存储当前相似度
def save_sim_f():
    np.savetxt(r'C:\Users\86138\Desktop\douban_w1\nofilter\douban_sim_final.txt', sim_f)


def save_f():
    print('正在存储过滤后的信任数据')
    q = ''
    with open(r'C:\Users\86138\Desktop\douban_w1\nofilter\douban_user_nei.txt', 'w+') as w:
        for i in tqdm(range(len(user_id))):
            if len(user[str(user_id[i])]['nei']) != 0:
